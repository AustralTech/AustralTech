        ...
        <div className="w-full flex flex-col h-screen overflow-y-hidden">
          ...
          <header className="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            ...
            <div className="relative w-1/2 flex justify-end">
              <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="relative z-10 w-12 h-12 rounded-full overflow-hidden border-4 border-gray-400 hover:border-gray-300 focus:border-gray-300 focus:outline-none">
                <img src="https://source.unsplash.com/uJ8LNVCBjFQ/400x400" alt="Profile" />
              </button>
              {isProfileOpen && (
                <>
                  <button onClick={() => setIsProfileOpen(false)} className="h-full w-full fixed inset-0 cursor-default"></button>
          ...
          <header className="w-full bg-sidebar py-5 px-6 sm:hidden">
            <div className="flex items-center justify-between">
              <a href="index.html" className="text-white text-3xl font-semibold uppercase hover:text-gray-300">Admin</a>
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-white text-3xl focus:outline-none">
                <i className={`${isSidebarOpen ? 'fas fa-times' : 'fas fa-bars'}`}></i>
              </button>
            </div>

            {/* <!-- Dropdown Nav --> */}
            <nav className={`${isSidebarOpen ? 'flex' : 'hidden'} flex-col pt-4`}>
